			...
			<Dialog.Content class="h-[calc(100vh-20rem)]">
				<div class="mx-auto w-full max-w-md overflow-scroll">
					<Dialog.Header>
						...
						<Dialog.Description
							>Change what fields are autofilled for data-input. Click anywhere to close and apply
							changes. Reloading the page will reset to defaults.</Dialog.Description
						>
					</Dialog.Header>
					<Tabs.Root value="company" class="mt-4">
							...
							<div class="my-2">
								...
								>
									...
									<Select.Content class="max-h-64 overflow-y-scroll">
										{#await streamed.firmTypes}
											<Loader2 class="h-6 w-6 animate-spin" />
										{:then firmTypes}
											{#each ['', ...firmTypes] as firmType}
												<Select.Item value={firmType}>{firmType ? firmType : 'None'}</Select.Item>
											{/each}
										{/await}
									</Select.Content>
								</Select.Root>
						...
						>
							...
							<div class="my-2">
								...
								>
									...
									<Select.Content class="max-h-64 overflow-y-scroll">
										{#await streamed.ethnicities}
											<Loader2 class="h-6 w-6 animate-spin" />
										{:then ethnicities}
											{#each ['', ...ethnicities].sort((a, b) => a.localeCompare(b)) as ethnicity}
												<Select.Item value={ethnicity}>{ethnicity ? ethnicity : 'None'}</Select.Item
												>
											{/each}
										{/await}
									</Select.Content>
						...
						>
							...
							<div class="my-2">
								...
								<p class="my-1 text-sm text-muted-foreground">Select a default seniority level</p>
								<Select.Root
									onSelectedChange={(v) => {
										if ($inputConfig.position && $inputConfig.position.fields) {
											if (v && v.value) {
								...
								<Popover.Root bind:open={openIMT} let:ids>
									...
									<Popover.Content class="w-full max-w-md p-0">
										<Command.Root>
											...
											<Command.Group class="max-h-64 overflow-y-scroll">
												{#await streamed.individualMoveTypes}
													<Loader2 class="h-6 w-6 animate-spin" />
